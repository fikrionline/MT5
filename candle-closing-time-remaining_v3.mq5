...
int OnCalculate(const int rates_total,
  ...
  string print_spread;
  if(displaySpread) {
    print_spread = " / Spread " + IntegerToString(SymbolInfoInteger(_Symbol, SYMBOL_SPREAD)) + " / PnL " + 
    DoubleToString(float_profit, 2);
  } else {
    print_spread = ""; 
  }
  ...
  if (DayOfWeek() == 0 || DayOfWeek() == 6) {
    ...
    if (Period() == PERIOD_MN1 || Period() == PERIOD_W1) {
      ...
      if (location == 0) {
        ...
        if (!displayServerTime) {
          ObjectSetString(0, "CandleClosingTimeRemaining-CCTR", OBJPROP_TEXT, OBJ_LABEL, (string) days + "D - " + sTime + print_spread);

        } else {
          ObjectSetString(0, "CandleClosingTimeRemaining-CCTR", OBJPROP_TEXT, OBJ_LABEL, (string) days + "D - " + sTime + " [" + sCurrentTime + "]" + print_spread);
      ...
      if (location == 0) {
        ...
        if (!displayServerTime) {
          ObjectSetString(0, "CandleClosingTimeRemaining-CCTR", OBJPROP_TEXT, OBJ_LABEL, sTime + print_spread);

        } else {
          ObjectSetString(0, "CandleClosingTimeRemaining-CCTR", OBJPROP_TEXT, OBJ_LABEL, sTime + " [" + sCurrentTime + "]" + print_spread);

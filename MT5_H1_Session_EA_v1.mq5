...
//| Fixed: MQL5 style order/position handling for MT5 compilation.    |
//+------------------------------------------------------------------+
...
void OnH1Close() {
   ...
   double highestOpen = GetTodaysHighestOpen();
   double lowestOpen = GetTodaysLowestOpen();
   if (highestOpen == 0 || lowestOpen == 0) {
      Print("[EA WARN] Could not determine today's highest/lowest open.");
      return;
   }
